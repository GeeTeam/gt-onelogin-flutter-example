        ...
        [OneLogin preGetTokenWithCompletion:^(NSDictionary * _Nonnull preResult) {
            ...
            if (preResult.count > 0 && preResult[@"status"] && 200 == [preResult[@"status"] integerValue]) {
                [OneLogin requestTokenWithViewController:controller viewModel:viewModel completion:^(NSDictionary * _Nullable result) {
                    ...
                    [wself finishRequestingToken:result flutterResult:flutterResult];
                }];
            } else {    // 预取号失败
                if (flutterResult) {
                    flutterResult(@"0");
...
- (void)finishRequestingToken:(NSDictionary *)result flutterResult:(FlutterResult)flutterResult {
    if (result.count > 0 && result[@"status"] && 200 == [result[@"status"] integerValue]) {
        ...
        [self validateTokenAndGetLoginInfo:token appID:appID processID:processID gwAuth:gwAuth  flutterResult:flutterResult];
    } else {
...
// 使用token获取用户的登录信息
- (void)validateTokenAndGetLoginInfo:(NSString *)token appID:(NSString *)appID processID:(NSString *)processID gwAuth:(NSString *)gwAuth flutterResult:(FlutterResult)flutterResult {
    // 根据用户自己接口构造
    // demo仅做演示
